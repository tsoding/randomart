...
FFMPEG *ffmpeg_start_rendering(size_t width, size_t height, size_t fps)
{
    ...
    snprintf(cmd_buffer, sizeof(cmd_buffer), "ffmpeg.exe -loglevel verbose -y -f rawvideo -pix_fmt rgba -s %dx%d -r %d -i - -c:v libx264 -vb 2500k -pix_fmt yuv420p output.mp4", (int)width, (int)height, (int)fps);
...
bool ffmpeg_send_frame(FFMPEG *ffmpeg, void *data, size_t width, size_t height)
{
    ...
    // TODO: handle ERROR_IO_PENDING
    if (!WriteFile(ffmpeg->hPipeWrite, data, sizeof(uint32_t)*width*height, &written, NULL)) {
        TraceLog(LOG_ERROR, "FFMPEG: failed to write into ffmpeg pipe. System Error Code: %d", GetLastError());
        return false;
    }
    return true;
